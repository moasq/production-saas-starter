...
export function adapterPlan(base) {
  ...
  const mcp = JSON.parse(read(base, ".mcp.json"));
  const expectedMcp = { mcpServers: { "better-auth": { type: "http", url: endpoint } } };
  if (JSON.stringify(mcp) !== JSON.stringify(expectedMcp)) throw new Error("MCP inventory differs from the reviewed read-only server");
  ...
  const outputs = new Map([
    ...
    ["next_b2b_starter/.claude/CLAUDE.md", "<!-- Generated by node scripts/harness.mjs sync. -->\n@../../AGENTS.md\n\nRead `.agents/skills/next-frontend/SKILL.md` from the repository root.\n"],
    [".codex/config.toml", `${generated}\n[mcp_servers.better-auth]\nurl = ${JSON.stringify(endpoint)}\nenabled_tools = ${JSON.stringify(manifest.mcp.allowedTools)}\n`],
  ]);
  for (const item of entries(base, ".agents/agents")) {
    ...
    const tools = ["Read", "Grep", "Glob", "WebFetch", ...(!readonly ? ["Edit", "Write", "Bash"] : []),
      ...manifest.mcp.allowedTools.map((name) => `mcp__better-auth__${name}`)];
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    if (process.argv.length !== 3) throw new Error("Usage: node scripts/harness.mjs sync|check|fetch-skills|check-mcp");
