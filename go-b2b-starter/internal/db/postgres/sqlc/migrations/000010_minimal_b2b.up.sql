...
CREATE TABLE IF NOT EXISTS organizations.accounts (
    id SERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_organizations_slug ON organizations.organizations(slug) WHERE status = 'active';
CREATE INDEX IF NOT EXISTS idx_organizations_status ON organizations.organizations(status);
CREATE INDEX IF NOT EXISTS idx_organizations_created_at ON organizations.organizations(created_at DESC);
CREATE UNIQUE INDEX IF NOT EXISTS idx_organizations_stytch_org_id ON organizations.organizations(stytch_org_id);

CREATE INDEX IF NOT EXISTS idx_accounts_org_id ON organizations.accounts(organization_id);
CREATE INDEX IF NOT EXISTS idx_accounts_email ON organizations.accounts(email);
CREATE INDEX IF NOT EXISTS idx_accounts_status ON organizations.accounts(status);
CREATE INDEX IF NOT EXISTS idx_accounts_role ON organizations.accounts(role);
CREATE INDEX IF NOT EXISTS idx_accounts_stytch_member_id ON organizations.accounts(stytch_member_id);
...
-- Keep historical roles readable while accepting the supported manager role.
-- Stytch is authoritative; local role fields are display metadata only.
ALTER TABLE organizations.accounts DROP CONSTRAINT IF EXISTS chk_accounts_role;
ALTER TABLE organizations.accounts DROP CONSTRAINT IF EXISTS valid_role_enum;
ALTER TABLE organizations.accounts ADD CONSTRAINT chk_accounts_role
 CHECK (role IN ('owner','admin','manager','member','approver','reviewer','employee'));
