...
export interface SubscriptionGateState {
  isAuthenticated: boolean; isActive: boolean; canStartCheckout: boolean; reason?: string; status?: string | null;
...
export async function resolveCurrentSubscription(): Promise<SubscriptionGateState> {
 ...
 const empty: SubscriptionGateState = { isAuthenticated: Boolean(session), isActive: false, canStartCheckout: false, productId: null, planId: null, subscription: null, backendAvailable: true };
 ...
 try {
   ...
   return { ...empty, isActive: status.HasActiveSubscription,
     canStartCheckout: status.BillingEnabled && !status.HasActiveSubscription && !status.SubscriptionID,
     reason: status.Reason, status: status.SubscriptionStatus,
